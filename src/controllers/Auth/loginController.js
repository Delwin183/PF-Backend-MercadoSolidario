  ...
  loginOfAUser: async function (body) {
    ...
    if (!user[0].isActive) {
      throw new Error(
        JSON.stringify({
          containErrors: true,
          message:
            "Por favor, espere y su registro único será validado, estaremos comunicandole via email el resultado de la validación, muchas gracias.",
        })
      );
    }
    ...
    if (!isEqual) {
      throw new Error(
        JSON.stringify({
          containErrors: true,
          message: "Contraseña incorrecta. Por favor, ingrese nuevamente.",
        })
      );
    }
